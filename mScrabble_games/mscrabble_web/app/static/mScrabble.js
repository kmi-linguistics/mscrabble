...
var lettersWeight =  JSON.parse(localStorage.getItem("lettersWeight"));
...
function selectRackRect(rX, rY, rackCount){
    ...
    // freeze rack button till a word is check
    document.getElementById(activePlayr).disabled = true;     
    ...
    if (word.length !== 0) {
        ...
        if (selectLetter === undefined) {
            ...
            // store letters for new word
            word.push(selectLetter);                               
...
document.getElementById("check").addEventListener("click", function() {
    ...
    $.getJSON('/searchWord', {
            a:String(wordsAroundNewLetter+','+language)
        }, function(data) {
            words(data.result);
        });
